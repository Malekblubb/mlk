		...
		namespace internal
		{
			...
			public:
				...
				void try_call(T error_code) const noexcept
				{
					...
					if(index != -1) m_cnt.get(index)->call();
				}
				...
				{
					int index{m_cnt.find(error_code)};
					if(index != -1) return m_cnt.get(index)->msg();

					return "";
				}

				template <typename T>
				void link_error(T error_code, const std::string& msg,
								const slot<>& fnc = {})
				{
				...
				cnt::error_type<T> get_error(T error_code) const noexcept
				{
					...
					if(index == -1)
						throw std::runtime_error{
							"mlk::[...]::error_handler: error_code not found"};

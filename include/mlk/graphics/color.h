	...
	namespace gcs
	{
		...
		public:
			...
			color_rgb(const Color_Type& type)
				: color_rgb{static_cast<std::uint8_t>(type.r),
							static_cast<std::uint8_t>(type.g),
							static_cast<std::uint8_t>(type.b),
							static_cast<std::uint8_t>(type.a)}
			{
			}

			color_rgb(std::uint8_t red, std::uint8_t green, std::uint8_t blue,
			...
			color_rgb(const std::string& hex_str) : m_hex{hex_str}
			{
		...
		private:
			...
			void generate_hex()
			{
				...
				strm << std::setfill('0') << std::hex << std::setw(2)
					 << static_cast<int>(m_r) << std::setw(2)
					 << static_cast<int>(m_g) << std::setw(2)
					 << static_cast<int>(m_b);
				m_hex = strm.str();
			}
		};

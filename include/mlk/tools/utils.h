...
namespace mlk
{
	...
	void exit_with(const std::string& reason, int exit_code = EXIT_FAILURE,
				   T& os = std::cerr)
	{
		os << reason;
		std::exit(exit_code);
	...
	inline bool parse_bool(const std::string& str)
	{
		...
		if(cpy.empty()) throw std::invalid_argument{"cant parse empty string"};
	...
	template <
		...
		typename Lambda_Return = typename mlk::lambda_info<Func>::return_type>

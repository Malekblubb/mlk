...
namespace mlk
{
	namespace tm
	{
		...
		inline auto sys_time_pnt() -> decltype(std::chrono::system_clock::now())
		{
			return std::chrono::system_clock::now();
		}

		template <typename T = milli_s, typename E>
		auto duration(const std::chrono::time_point<E>& start,
					  const std::chrono::time_point<E>& end)
		...
		auto duration_as(const std::chrono::time_point<M>& start,
						 const std::chrono::time_point<M>& end)
		...
		template <typename T, typename E = std::milli, typename M>
		auto duration_to_now_as(const std::chrono::time_point<M>& start)
			-> decltype(duration_as<T, E, M>(start, time_pnt()))
		{
			return duration_as<T, E, M>(start, time_pnt());
		}
		...
		bool timed_out(
			const decltype(
				(std::chrono::high_resolution_clock::now())) start_pnt,
			llong max)
		{

...
namespace mlk
{
	namespace cnt
	{
		namespace internal
		{
			...
			void make_vector_impl(std::vector<Vec_Type>& result,
								  const Vec_Type& head)
			{
		...
		template <typename T>
		bool vec_cmp(std::size_t from, std::size_t to,
		...
		{
			for(auto& a : find_one)
				if(find_in(a, in) != in.end()) return true;
			return false;
		}
		...
		{
			for(auto& a : vec)
				if(exists_multiple(a, vec)) remove_but(1, a, vec);
		}
		...
		{
			cnt.erase(std::remove_if(std::begin(cnt), std::end(cnt), p),
					  std::end(cnt));
		}
		...
		void map_remove_if(Pred&& pred, Map_Type& map)
		{
			...
			while((iter = std::find_if(iter, std::end(map), pred)) !=
				  std::end(map))
				map.erase(iter++);
		}
		...
		template <typename Vec_Type, typename... Args>
		auto make_vector(Args&&... args) -> std::vector<Vec_Type>
		{

...
namespace mlk
{
	namespace cnt
	{
		...
		public:
			template <typename T>
			void link(T error_code, const std::string& msg,
					  const mlk::slot<>& fnc = {})
			{
				m_errors.push_back(
					std::make_shared<error_type<T>>(error_code, msg, fnc));
			}
			...
			int find(T error_code) const
			{
				int index{0};
				for(auto& a : m_errors) {
					if(std::static_pointer_cast<error_type<T>>(a)->cmp_code(
						   error_code))
						return index;
					++index;
				}
			...
			std::shared_ptr<error_type<T>> get_casted(int index) const
			{
				if(is_out_of_bounds(m_errors, index))
					throw std::out_of_range{
						"mlk::cnt::error_container::get_casted: index was out "
						"of bounds"};
			...
			std::shared_ptr<internal::error_type_base> get(int index) const
			{
				if(is_out_of_bounds(m_errors, index))
					throw std::out_of_range{
						"mlk::cnt::error_container::get: index was out of "
						"bounds"};
